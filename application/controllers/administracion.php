...
class Administracion extends CI_Controller {
	...
	public function index()
	{
            ...
            $this->load->view('administracion',$data2);
        ...
        {
         $this->load->library('session');
         $comprobacion = $this->session->userdata('usuario');
         if($comprobacion == "admin")
         {
                $get = $this->uri->uri_to_assoc();
                $producto = $get['pdf']; // valor   
                $usuario = $get['usuario']; // valor                   
                $this->load->database();
        ...
        public function crear_factura()
        {
            ...
            echo form_open('administracion/introducir_factura');
            echo("Selecciona Usuario: </br>");
            $this->load->database();
            $query = $this->db->query('SELECT DISTINCT nombre from usuarios');
            if ($query->num_rows() > 0)
            {
                echo('<select name="select_usuario>');
                $i = 0;
                foreach ($query->result() as $row)
                {
                    echo('<option value="'.$i.'">');
        ...
        public function obtener_facturas()
        {
         ...
         {
            $this->load->database();
            $query = $this->db->query('SELECT facturacion.referencia referencia, proyectos.nombre nombre, usuario_factura, fecha,precio,metodo_pago,Pagada from facturacion,proyectos where facturacion.proyecto = proyectos.referencia');
            if ($query->num_rows() > 0)
            {
                ...
                echo('<td> <b> Usuario Factura   </b> </td>');
                echo('<td> <b> Fecha   </b> </td>');
                echo('<td> <b> Precio   </b> </td>');
                echo('<td> <b> Método de Pago   </b> </td>');
                echo('<td> <b> Pagada </b> </td>');
                echo('<td> <b> Descargar </b> </td>');
                echo('<td> <b> Cobrar </b> </td>');
                ...
                foreach ($query->result() as $row)
                {
                    ...
                    echo("<td>" .$row->usuario_factura. "</td>");
                    echo("<td>" .$row->fecha. "</td>");
                    echo("<td>" .$row->precio. "</td>");
                    echo("<td>" .$row->metodo_pago. "</td>");
                    echo("<td>" .$row->Pagada. "</td>");
                    echo("<td> <a href=\"https://www.loadingstudios.archive.tornadocore.es/administracion/obtener_pdf/pdf/".$row->referencia."/usuario/".$row->usuario_factura."/\" style=\"color:white;\">Descargar</a></td>");
                    
                    if($row->Pagada == 0)
                    {
                    echo("<td> <a href=\"https://www.loadingstudios.archive.tornadocore.es/administracion/obtener_pdf/pdf/".$row->referencia."/usuario/".$row->usuario_factura."/\" style=\"color:white;\">Establecer Pagada</a></td>");
                    }
                    else
                    {
                        echo("PAGADA");
                ...
                echo ('<a href="https://www.loadingstudios.archive.tornadocore.es/administracion/crear_factura/">Crear Nueva Factura</a>');
                echo ('<a href="#">Eliminar Factura</a>');
                
            }
         }
         else
             echo ("a donde vas? jeje");
        }
